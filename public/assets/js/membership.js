...
const PAYMENT_SERVER_URL = window.location.href.indexOf('localhost') > 0 ? 'http://localhost:8888' : 'https://flixi.org';
...
function getMembershipDetails(callback) {
  ...
  function enforceMembershipForContentPages() {
   ...
   //is expiring soon
   const isExpiringSoon = (membershipDetailsResult) => {
    const now = new Date();
    const expiry = new Date(membershipDetailsResult.details.expiry);
    const diff = expiry - now;
    return diff < 86400 * 1000 * 3; // 24 hours in milliseconds
  }
